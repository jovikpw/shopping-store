                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-1 col-md-1 col-sm-8 col-xs-4">
                            <form action="{{ route('cart.destroy', $cartItem->rowId) }}" method="post">
                                ...
                                <button onclick="return confirm('Вы уверены?')" class="btn btn-danger btn-sm"><i class="fa fa-times"></i></button>
                            </form>
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                            <span class="hidden-lg hidden-md"><small>Налог: </span>
                            {{config('cart.currency')}} {{ number_format(($cartItem->qty*$cartItem->tax), 2) }}</small>
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                            <span class="hidden-lg hidden-md"><small>Промежуточный итог: </span>
                            {{config('cart.currency')}} {{ number_format($cartItem->price, 2) }}</small>
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                            <span class="hidden-lg hidden-md"><small>Итого: </span>
